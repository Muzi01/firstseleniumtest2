...
import bindings.cucumber.funcjonal.TimeoutException;
...
public class Utils {
    ...
    public static void waitUntil(BooleanSupplier condition, long delay, long timeout) {
        if (!isConditionMet(condition, delay, timeout)) {
            throw new TimeoutException(String.format("Condition not met in %d seconds!", timeout / 1000L));
        }
    }

    public static void waitUntil(BooleanSupplier condition, long delay, long timeout, String errorMsg) {
        if (!isConditionMet(condition, delay, timeout)) {
            throw new TimeoutException(String.format("Condition not met in %d seconds! <br> %s", timeout / 1000L, errorMsg));
        }
    }
