
package bindings.commons.automation.mule.model.mule.development.application.sfdc.channel;

import com.fasterxml.jackson.annotation.JsonAnyGetter;
import com.fasterxml.jackson.annotation.JsonAnySetter;
import com.fasterxml.jackson.annotation.JsonIgnore;
import com.fasterxml.jackson.annotation.JsonInclude;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonPropertyOrder;
import org.apache.commons.lang3.builder.EqualsBuilder;
import org.apache.commons.lang3.builder.HashCodeBuilder;
import org.apache.commons.lang3.builder.ToStringBuilder;

import java.util.HashMap;
import java.util.Map;

@JsonInclude (JsonInclude.Include.NON_NULL)
@JsonPropertyOrder ({
    "backendId",
    "name",
    "principal",
    "invoicingFee",
    "openingFee",
    "interest",
    "apr",
    "minimumWithdrawalPercentage",
    "minimumWithdrawalFee",
    "withdrawalFeePercentage",
    "withdrawalFixedFee",
    "hidden",
    "country",
    "type",
    "requiredScore",
    "requiredScoreRepeat",
    "brand",
    "groupName"
})
public class Product {

  @JsonProperty ("backendId")
  private String backendId;
  @JsonProperty ("name")
  private String name;
  @JsonProperty ("principal")
  private String principal;
  @JsonProperty ("invoicingFee")
  private String invoicingFee;
  @JsonProperty ("openingFee")
  private String openingFee;
  @JsonProperty ("interest")
  private String interest;
  @JsonProperty ("apr")
  private String apr;
  @JsonProperty ("minimumWithdrawalPercentage")
  private String minimumWithdrawalPercentage;
  @JsonProperty ("minimumWithdrawalFee")
  private String minimumWithdrawalFee;
  @JsonProperty ("withdrawalFeePercentage")
  private String withdrawalFeePercentage;
  @JsonProperty ("withdrawalFixedFee")
  private String withdrawalFixedFee;
  @JsonProperty ("hidden")
  private String hidden;
  @JsonProperty ("country")
  private String country;
  @JsonProperty ("type")
  private String type;
  @JsonProperty ("requiredScore")
  private String requiredScore;
  @JsonProperty ("requiredScoreRepeat")
  private String requiredScoreRepeat;
  @JsonProperty ("brand")
  private String brand;
  @JsonProperty ("groupName")
  private String groupName;
  @JsonIgnore
  private final Map<String, Object> additionalProperties = new HashMap<>();

  @JsonProperty ("backendId")
  public String getBackendId() {
    return this.backendId;
  }

  @JsonProperty ("backendId")
  public void setBackendId(final String backendId) {
    this.backendId = backendId;
  }

  @JsonProperty ("name")
  public String getName() {
    return this.name;
  }

  @JsonProperty ("name")
  public void setName(final String name) {
    this.name = name;
  }

  @JsonProperty ("principal")
  public String getPrincipal() {
    return this.principal;
  }

  @JsonProperty ("principal")
  public void setPrincipal(final String principal) {
    this.principal = principal;
  }

  @JsonProperty ("invoicingFee")
  public String getInvoicingFee() {
    return this.invoicingFee;
  }

  @JsonProperty ("invoicingFee")
  public void setInvoicingFee(final String invoicingFee) {
    this.invoicingFee = invoicingFee;
  }

  @JsonProperty ("openingFee")
  public String getOpeningFee() {
    return this.openingFee;
  }

  @JsonProperty ("openingFee")
  public void setOpeningFee(final String openingFee) {
    this.openingFee = openingFee;
  }

  @JsonProperty ("interest")
  public String getInterest() {
    return this.interest;
  }

  @JsonProperty ("interest")
  public void setInterest(final String interest) {
    this.interest = interest;
  }

  @JsonProperty ("apr")
  public String getApr() {
    return this.apr;
  }

  @JsonProperty ("apr")
  public void setApr(final String apr) {
    this.apr = apr;
  }

  @JsonProperty ("minimumWithdrawalPercentage")
  public String getMinimumWithdrawalPercentage() {
    return this.minimumWithdrawalPercentage;
  }

  @JsonProperty ("minimumWithdrawalPercentage")
  public void setMinimumWithdrawalPercentage(final String minimumWithdrawalPercentage) {
    this.minimumWithdrawalPercentage = minimumWithdrawalPercentage;
  }

  @JsonProperty ("minimumWithdrawalFee")
  public String getMinimumWithdrawalFee() {
    return this.minimumWithdrawalFee;
  }

  @JsonProperty ("minimumWithdrawalFee")
  public void setMinimumWithdrawalFee(final String minimumWithdrawalFee) {
    this.minimumWithdrawalFee = minimumWithdrawalFee;
  }

  @JsonProperty ("withdrawalFeePercentage")
  public String getWithdrawalFeePercentage() {
    return this.withdrawalFeePercentage;
  }

  @JsonProperty ("withdrawalFeePercentage")
  public void setWithdrawalFeePercentage(final String withdrawalFeePercentage) {
    this.withdrawalFeePercentage = withdrawalFeePercentage;
  }

  @JsonProperty ("withdrawalFixedFee")
  public String getWithdrawalFixedFee() {
    return this.withdrawalFixedFee;
  }

  @JsonProperty ("withdrawalFixedFee")
  public void setWithdrawalFixedFee(final String withdrawalFixedFee) {
    this.withdrawalFixedFee = withdrawalFixedFee;
  }

  @JsonProperty ("hidden")
  public String getHidden() {
    return this.hidden;
  }

  @JsonProperty ("hidden")
  public void setHidden(final String hidden) {
    this.hidden = hidden;
  }

  @JsonProperty ("country")
  public String getCountry() {
    return this.country;
  }

  @JsonProperty ("country")
  public void setCountry(final String country) {
    this.country = country;
  }

  @JsonProperty ("type")
  public String getType() {
    return this.type;
  }

  @JsonProperty ("type")
  public void setType(final String type) {
    this.type = type;
  }

  @JsonProperty ("requiredScore")
  public String getRequiredScore() {
    return this.requiredScore;
  }

  @JsonProperty ("requiredScore")
  public void setRequiredScore(final String requiredScore) {
    this.requiredScore = requiredScore;
  }

  @JsonProperty ("requiredScoreRepeat")
  public String getRequiredScoreRepeat() {
    return this.requiredScoreRepeat;
  }

  @JsonProperty ("requiredScoreRepeat")
  public void setRequiredScoreRepeat(final String requiredScoreRepeat) {
    this.requiredScoreRepeat = requiredScoreRepeat;
  }

  @JsonProperty ("brand")
  public String getBrand() {
    return this.brand;
  }

  @JsonProperty ("brand")
  public void setBrand(final String brand) {
    this.brand = brand;
  }

  @JsonProperty ("groupName")
  public String getGroupName() {
    return this.groupName;
  }

  @JsonProperty ("groupName")
  public void setGroupName(final String groupName) {
    this.groupName = groupName;
  }

  @Override
  public String toString() {
    return ToStringBuilder.reflectionToString(this);
  }

  @JsonAnyGetter
  public Map<String, Object> getAdditionalProperties() {
    return this.additionalProperties;
  }

  @JsonAnySetter
  public void setAdditionalProperty(final String name, final Object value) {
    this.additionalProperties.put(name, value);
  }

  @Override
  public int hashCode() {
    return new HashCodeBuilder().append(this.backendId).append(this.name).append(this.principal)
        .append(this.invoicingFee).append(this.openingFee).append(this.interest).append(this.apr)
        .append(this.minimumWithdrawalPercentage).append(this.minimumWithdrawalFee)
        .append(this.withdrawalFeePercentage).append(this.withdrawalFixedFee).append(this.hidden)
        .append(
            this.country)
        .append(this.type).append(this.requiredScore).append(this.requiredScoreRepeat).append(
            this.brand)
        .append(this.groupName).append(this.additionalProperties).toHashCode();
  }

  @Override
  public boolean equals(final Object other) {
    if (other == this) {
      return true;
    }
    if (!(other instanceof Product)) {
      return false;
    }
    final Product rhs = ((Product) other);
    return new EqualsBuilder().append(this.backendId, rhs.backendId).append(this.name, rhs.name)
        .append(this.principal, rhs.principal).append(this.invoicingFee, rhs.invoicingFee)
        .append(this.openingFee, rhs.openingFee).append(this.interest, rhs.interest).append(
            this.apr, rhs.apr)
        .append(this.minimumWithdrawalPercentage, rhs.minimumWithdrawalPercentage)
        .append(this.minimumWithdrawalFee, rhs.minimumWithdrawalFee)
        .append(this.withdrawalFeePercentage, rhs.withdrawalFeePercentage)
        .append(this.withdrawalFixedFee, rhs.withdrawalFixedFee).append(this.hidden, rhs.hidden)
        .append(this.country, rhs.country).append(this.type, rhs.type)
        .append(this.requiredScore, rhs.requiredScore)
        .append(this.requiredScoreRepeat, rhs.requiredScoreRepeat).append(this.brand, rhs.brand)
        .append(this.groupName, rhs.groupName)
        .append(this.additionalProperties, rhs.additionalProperties)
        .isEquals();
  }

}
